...
class FaceAI():
    def __init__(self, minDetectConf=0.5):
        ...
        self.FaceDetector = self.mpFaceDetection.FaceDetection(self.minDetectConf)
        self.mpDraw = mp.solutions.drawing_utils
    def faceDetection(self, record):
        results = self.FaceDetector.process(cv2.cvtColor(record, cv2.COLOR_BGR2RGB))
        circleDrawingSpec = self.mpDraw.DrawingSpec(thickness=3, circle_radius=1, color=(0, 255, 0))
        ...
        if results.detections:
            for id, detection in enumerate(results.detections):
                ...
                cv2.putText(record, f'{int(detection.score[0]*100)}%', (bbox[0],bbox[1]-20), cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 0), 2)
def FaceDet():
